...
void hardClass::drawEnemy(int ind) {
	...
	if(enemyFlag[ind] == true) {
		...
		gotoxy(enemyX[ind], enemyY[ind]);   cout << "1**1";
		gotoxy(enemyX[ind], enemyY[ind]+1); cout << " ** ";
		gotoxy(enemyX[ind], enemyY[ind]+2); cout << "1**1";
...
void hardClass::hardPlay() {
	...
	while(1) {
		...
		if (enemyY[0] == 10)
			...
			if (score == 20) {
        system("cls");
        gotoxy(WIN_WIDTH / 2 - 5, SCREEN_HEIGHT / 2);
        cout << "YOU WIN!";
        gotoxy(WIN_WIDTH / 2 - 15, SCREEN_HEIGHT / 2 + 2);
